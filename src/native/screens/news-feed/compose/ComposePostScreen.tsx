...
const authorAvatar = assetManifest.app.studentAvatar;
...
export function ComposePostScreen({ authorName, colors, onClose, onSubmit }: ComposePostScreenProps) {
  ...
  const quickActions: QuickAction[] = [
    { icon: Music2, label: t('newsFeed.compose.quickActions.music'), tint: '#B923F5' },
    { icon: UserRoundPlus, label: t('newsFeed.compose.quickActions.tag'), tint: colors.primaryDark },
    { icon: Smile, label: t('newsFeed.compose.quickActions.feeling'), tint: colors.success },
    { icon: MapPin, label: t('newsFeed.compose.quickActions.location'), tint: '#A020F0' },
  ];
  ...
  return (
    ...
    >
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text numberOfLines={1} style={[styles.headerTitle, { color: colors.text }]}>{t('newsFeed.compose.title')}</Text>

        <Pressable
          accessibilityLabel={t('newsFeed.compose.title')}
          accessibilityRole="button"
          onPress={onSubmit}
        ...
        >
          <LinearGradient colors={[colors.primaryDark, colors.primaryHover]} style={styles.publishGradient}>
            <Text style={styles.publishText}>{t('newsFeed.compose.publish')}</Text>
          </LinearGradient>
        </Pressable>
      </View>
      ...
      >
        <View style={styles.authorRow}>
          ...
          <View style={styles.authorCopy}>
            <Pressable
              accessibilityLabel={t('newsFeed.compose.chooseIdentity')}
              accessibilityRole="button"
            ...
            <Pressable
              accessibilityLabel={t('newsFeed.compose.chooseReplyAudience')}
              accessibilityRole="button"
              style={({ pressed }) => [
                styles.replyPill,
            ...
            >
              ...
              <Text numberOfLines={1} style={[styles.replyText, { color: colors.textSecondary }]}>{t('newsFeed.compose.replyAudienceEveryone')}</Text>
        ...
        <View style={[styles.editorCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <TextInput
            accessibilityLabel={t('newsFeed.compose.editorLabel')}
            multiline
            onChangeText={setContent}
            placeholder={t('newsFeed.compose.placeholder')}
        ...
        <View style={[styles.addCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={[styles.sectionTitle, { color: colors.text }]}>{t('newsFeed.compose.addToPost')}</Text>
          <View style={styles.attachmentGrid}>
            <AttachmentOption colors={colors} label={t('newsFeed.compose.attachment.photoVideo')} tint={colors.primaryDark}>
              <ImageIcon color={colors.primaryDark} size={26} strokeWidth={2.1} />
            </AttachmentOption>
            <AttachmentOption colors={colors} label={t('newsFeed.compose.attachment.gif')} tint={colors.success}>
              <Text style={[styles.gifText, { color: colors.success }]}>GIF</Text>
            </AttachmentOption>
            <AttachmentOption colors={colors} label={t('newsFeed.compose.attachment.live')} tint="#F02B63">
              <Radio color="#F02B63" size={26} strokeWidth={2.1} />
            </AttachmentOption>
            <AttachmentOption colors={colors} label={t('newsFeed.compose.attachment.smartContract')} tint={colors.warning}>
        ...
        <Pressable
          accessibilityLabel={t('newsFeed.compose.openAdvanced')}
          accessibilityRole="button"
          style={({ pressed }) => [
            styles.advancedRow,
        ...
        >
          ...
          <Text style={[styles.advancedText, { color: colors.text }]}>{t('newsFeed.compose.advanced')}</Text>
...
function AttachmentOption({
  ...
  return (
    <Pressable
      accessibilityLabel={t('newsFeed.compose.addAttachment', { label })}
      accessibilityRole="button"
